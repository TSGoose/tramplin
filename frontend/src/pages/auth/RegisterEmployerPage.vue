  ...
  <section class="page-section">
    <UiContainer className="max-w-2xl">
      <UiCard className="p-8">
        <div class="mb-8">
          <h1 class="text-2xl font-semibold text-slate-900">Регистрация работодателя</h1>
        ...
        <form class="grid gap-5 md:grid-cols-2" @submit.prevent="onSubmit">
          <UiField label="Имя представителя" :error="errors.display_name">
            <UiInput v-model="form.display_name" placeholder="Анна Смирнова" autocomplete="name" />
          </UiField>

          <UiField label="Email" :error="errors.email">
            <UiInput v-model="form.email" type="email" placeholder="hr@company.ru" autocomplete="email" />
          </UiField>
          ...
          <div class="md:col-span-2">
            <UiButton type="submit" :disabled="authStore.isLoading" fullWidth>
              {{ authStore.isLoading ? 'Создание...' : 'Создать аккаунт работодателя' }}
            </UiButton>
          </div>
        </form>
      </UiCard>
...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    await authStore.registerEmployer({
